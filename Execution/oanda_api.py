...
class OandaAPI():
    ...
    def place_trade(self, pair, units, time, take_profit=None, stop_loss=None):
        url = f"{defs.OANDA_URL}/accounts/{defs.ACCOUNT_ID}/orders"
        ...
        if "orderFillTransaction" in json_data and "tradeOpened" in json_data["orderFillTransaction"]:
            ...
            desired_price = float(json_data["orderFillTransaction"]["fullVWAP"])
            if units > 0:
                actual_price = float(json_data["orderFillTransaction"]["fullPrice"]["asks"][0]["price"])
            else:
                actual_price = float(json_data["orderFillTransaction"]["fullPrice"]["bids"][0]["price"])
            actual_time = json_data["orderFillTransaction"]["time"]
        ...
        return dict(Id=trade_id, wanted_price=desired_price, actual_price=actual_price,
                    wanted_time=time, actual_time=actual_time, status=ok)
