...
from backtest import run_test, hourly_to_daily, apply_strat, plot_equity_line
import pandas as pd
...
EURUSD, AUDUSD, GBPUSD, JPYUSD, CNYUSD, USDCAD = ((EUR, USD), (AUD, USD), (GBP, USD), 
                                        (JPY, USD), (CNY, USD), (USD, CAD))
...
def daily_test(pair, plot_equity=False):
    df = get_data_av(pair, 'full', format_for='manual')
    df_daily = hourly_to_daily(df)
    name = '{}/{}'.format(*pair)
    run_test(df_daily, 'RSI2, Low5 for {}'.format(name), 1_000, plot_equity=plot_equity)
    return df_daily

#cnyusd = daily_test(CNYUSD, plot_equity=False)

opt_universe = {"low_pd":[3,7,1], "rsi_pd":[2,6,1], "rsi_threshold":[40,71,10], "max_dim":[5,6,1]}

euro_dollar = pd.read_csv('Data/Old/EUR_USD.csv').drop(["Vol.", "Change %"], axis=1)[::-1]
aud_dollar = pd.read_csv('Data/Old/AUD_USD.csv').drop(["Vol.", "Change %"], axis=1)[::-1]
gbp_dollar = pd.read_csv('Data/Old/GBP_USD.csv').drop(["Vol.", "Change %"], axis=1)[::-1]
